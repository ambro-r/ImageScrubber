 ...
 *
 * <p>Taken from
 * https://stackoverflow.com/questions/16602807/programmatically-determine-if-2-images-look-the-same-using-java
 * Credit to : https://stackoverflow.com/users/473290/eric
 */
public class ImageUtils_TEMp {

  private static final Logger logger = LoggerFactory.getLogger(ImageUtils_TEMp.class);
    ...
    for (int i = 0; i < length; i++) {
      if (data1[i] != data2[i]) {
        ...
        if ((((data1[i] >> 24) & 0xff) == 0) && (((data2[i] >> 24) & 0xff) == 0)) {
          logger.debug(
              "Both pixles at spot {} are different but 100% transparent.", Integer.valueOf(i));
        } else {
  ...
  private static final int[] getPixels(final BufferedImage img, final File file) {
    ...
    if (img.getColorModel().getColorSpace() == ColorSpace.getInstance(ColorSpace.CS_sRGB)) {
      ...
      pixelImg =
          new ColorConvertOp(ColorSpace.getInstance(ColorSpace.CS_sRGB), null).filter(img, null);
    }
  ...
  static final BufferedImage getBufferedImage(final File file) {
    Image image;
  ...
  /**
   * Compares file1 to file2 to see if they are the same based on a visual pixel by pixel
   * comparison. This has issues with marking images different when they are not. Works perfectly
   * for all images.
   *
   ...
   */
  private static final boolean visuallyCompareJava(final File file1, final File file2) {
    return equals(
        getPixels(getBufferedImage(file1), file1), getPixels(getBufferedImage(file2), file2));
  }
   ...
   */
  public static final boolean visuallyCompare(final File file1, final File file2) {
    ...
    if (!answer) {
      logger.info(
          "The files \"{}\" and \"{}\" are not pixel by pixel the same image. Manual comparison required.",
          file1.getPath(),
          file2.getPath());
    }
   ...
   * @return <code>true</code> if the image contains one or more pixels with some percentage of
   *     transparency (Alpha)
   */
  public static final boolean containsAlphaTransparency(final File file) {
